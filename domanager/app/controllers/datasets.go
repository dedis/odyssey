...
func datasetsPost(w http.ResponseWriter, r *http.Request,
	...
	go func() {
		...
		if conf.Standalone {
			...
			outb, err := conf.Executor.Run("./bcadmin", "darc", "add",
				"--id", session.Cfg.AdminIdentity.String(),
				"--desc", desc, "-bc", session.BcPath, "--shortPrint",
				"--unrestricted")
			if err != nil {
				task.CloseError(tef.Source, "create new darc", err.Error())
				return
			}
			...
			// spawn:calypsoWrite
			outb, err = conf.Executor.Run("./bcadmin", "-c", conf.ConfigPath, "darc",
				"rule", "--rule", "spawn:calypsoWrite",
				"--id", session.Cfg.AdminIdentity.String(),
				"-bc", session.BcPath, "--darc", newDarcID)
			if err != nil {
				task.CloseError(tef.Source, "spawn:calypsoRead", err.Error())
				return
			}
		...
		conf.CloudClient.PutObject("datasets", newFileName, ciphertextReader, -1,
		...
		identityStr := session.Cfg.AdminIdentity.String()
		args := []string{"./csadmin", "-c", conf.ConfigPath, "contract",
		...
		outb, err := conf.Executor.Run(args...)
		log.Info(fmt.Sprintf("command executed: %s", args))
		if err != nil {
			task.CloseError(tef.Source, "csadmin failed", err.Error())
			return
		}
		...
		args = []string{"./catadmin", "-c", conf.ConfigPath, "contract",
		...
		task.AddInfof(tef.Source, "updating the catalog",
			"using the following command: %v", args)

		log.Info(fmt.Sprintf("command executed: ./catadmin %s", args))
		if err != nil {
			task.CloseError(tef.Source, "update catalog failed", err.Error())
			return
		}
