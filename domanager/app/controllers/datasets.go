...
func datasetsPost(w http.ResponseWriter, r *http.Request,
	...
	go func() {
		...
		task.AddInfo(tef.Source, "creating a Calypso write", "using csadmin to created the calypso write that contains the symetric key and the nonce")
		identityStr := session.Cfg.AdminIdentity.String()
		cmd := exec.Command("./csadmin", "-c", conf.ConfigPath, "contract", "write",
			"spawn", "--darc", darcPostResponse.DarcID, "--sign", identityStr, "--bc",
...
func datasetsShow(w http.ResponseWriter, r *http.Request,
	...
	darcMatch, err := regexp.Compile(`DarcID: ([0-9a-f]{64})`)
	if err != nil {
		xhelpers.RedirectWithErrorFlash("/", "failed to create the DARC regex: "+
			err.Error(), w, r, store)
		return
	}
	...
	readAttrRule, err := regexp.Compile(
		...
		`spawn:calypsoRead - ` +
			...
			`.*`)
	if err != nil {
		xhelpers.RedirectWithErrorFlash("/", "failed to create the readAttrRule "+
			"regex: "+err.Error(), w, r, store)
		return
	}
	...
	// we have everythign we need, let's render the view
...
func datasetsShowAttributesPut(w http.ResponseWriter, r *http.Request,
	...
	go func() {
		...
		darcMatch, err := regexp.Compile(`DarcID: ([0-9a-f]{64})`)
		if err != nil {
			task.CloseError(tef.Source, "failed to create the DARC regex: ",
				err.Error())
			return
		}
		match := darcMatch.FindStringSubmatch(output)
		...
		readRule, err := regexp.Compile(`spawn:calypsoRead - "(.*)"`)
		if err != nil {
			task.CloseError(tef.Source, "failed to create the readRule "+
				"regex", err.Error())
			return
		}
		...
		attrRule, err := regexp.Compile(`.*\b*(\(\s*attr:allowed:[^\s]*\s*&\s*attr:must_have:[^\s]*\s*\))`)
		if err != nil {
			task.CloseError(tef.Source, "failed to create the attrRule "+
				"regex", err.Error())
			return
		}
