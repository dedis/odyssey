...
func datasetsPost(w http.ResponseWriter, r *http.Request,
	...
	go func() {
		...
		if conf.Standalone {
			...
			// Create a darc to control access to the calypsoWriteInstance. For the moment,
			// the only rule on it allows evolution by the admin Darc from the current bc.cfg file.
			...
			cmd := exec.Command("./bcadmin", "darc", "add",
				...
				"--desc", desc, "-bc", session.BcPath, "--shortPrint", "--unrestricted")
			var outb, errb bytes.Buffer
			cmd.Stdout = &outb
			cmd.Stderr = &errb
			err = cmd.Run()
			if err != nil {
				task.CloseError(tef.Source, "create new darc", fmt.Sprintf("failed to create a new DARC: %s - "+
			...
			if len(outputSplit) != 2 {
				task.CloseError(tef.Source, "parse new darc", fmt.Sprintf("expected the output split to "+
					"have 2 elements, but foud %d: %v", len(outputSplit), outputSplit))
				return
			}
			...
			if !ok {
				task.CloseError(tef.Source, "regexp parse new darc", fmt.Sprintf("regex match failed: '%s' does "+
					"not match the darc regex", newDarcID))
				return
			}
			...
			err = cmd.Run()
			if err != nil {
				task.CloseError(tef.Source, "spawn:calypsoRead", fmt.Sprintf("failed to add rule "+
			...
			task.AddInfof(tef.Source, "Darc created in standalone mode", "DarcID: %v", newDarcID)
		} else {
			// Getting a DARC to spawn a new calypso write instance from the enclave
			// manager. Note that there is no identity check from the enclave manager
			// yet, so anyone can request a DARC
			formData := url.Values{
				"darcID": {session.Cfg.AdminDarc.GetIdentityString()},
			}
			task.AddInfof(tef.Source, "getting a new DARC from the enclave manager", "calling POST http://localhost:5000/darcs with %v", formData)
		...
		task.AddInfo(tef.Source, "getting the dataset file", "retrieving the 'dataset-file' post argument")
		...
		// Generating the symetric key and the initialization value
		// We need 16 bytes for the key and 12 bytes for the initialization value
		task.AddInfo(tef.Source, "genering a symmetric key", "genering a 16 bytes symmetric key")
		key := make([]byte, 16)
		_, err = rand.Read(key)
		if err != nil {
		...
		task.AddInfo(tef.Source, "encrypting the dataset", "encrypting with AES using the Galois Counter Mode")
		...
		task.AddInfof(tef.Source, "uploading the encrypted dataset on the cloud", "saving the encrypted dataset at %s", cloudURL)
		...
		task.AddInfo(tef.Source, "computing the SHA2", "using the unencrypted file to compute the SHA2")
		h := sha256.New()
		h.Write(fileBuf)
		sha2 := hex.EncodeToString(h.Sum(nil))

		// Creating the calypso write. We need to store the cloud URL because the
		// enclave will get it by parsing the extra data of the write instance with
		// `perl -n -e '/"CloudURL": "(.*?)",/ && print $1'`
		task.AddInfo(tef.Source, "creating a Calypso write", "using csadmin to create the calypso write that contains the symetric key and the nonce")
		identityStr := session.Cfg.AdminIdentity.String()
		cmd := exec.Command("./csadmin", "-c", conf.ConfigPath, "contract", "write",
			"spawn", "--darc", newDarcID, "--sign", identityStr, "--bc",
		...
		err = cmd.Run()
		if err != nil {
			task.CloseError(tef.Source, "csadmin failed", fmt.Sprintf("failed to spawn a "+
				"write instance: %s - Output: %s - Err: %s", err.Error(),
		...
		task.AddInfo(tef.Source, "getting the write instance ID", "parsing the output of csadmin to extract the write instance ID")
		...
		task.AddInfof(tef.Source, "updating the catalog", "using the following command: %v", cmd.Args)
